...
class ReactNativeBiometrics: RCTEventEmitter {
  ...
  @objc
  func startBiometricChangeDetection(
  ...
  @objc
  func stopBiometricChangeDetection(
    _ resolve: @escaping RCTPromiseResolveBlock,
    rejecter reject: @escaping RCTPromiseRejectBlock
  ) {
  ...
  func isSensorAvailable(
    ...
    if context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &error) {
      ...
      ReactNativeBiometricDebug.debugLog(
        "isSensorAvailable result: available=true, biometryType=\(biometryType)")
  ...
  func simplePrompt(
    ...
    if context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: nil) {
      ...
      context.evaluatePolicy(
        .deviceOwnerAuthenticationWithBiometrics, localizedReason: reason as String
      ) { success, error in
  ...
  func authenticateWithOptions(
    ...
    ReactNativeBiometricDebug.debugLog(
      "Authentication options - title: \(title), allowDeviceCredentials: \(allowDeviceCredentials), disableDeviceFallback: \(disableDeviceFallback)"
    )
    ...
    let policy: LAPolicy =
      allowDeviceCredentials ? .deviceOwnerAuthentication : .deviceOwnerAuthenticationWithBiometrics

    ReactNativeBiometricDebug.debugLog(
      "Using authentication policy: \(policy == .deviceOwnerAuthentication ? "deviceOwnerAuthentication" : "deviceOwnerAuthenticationWithBiometrics")"
    )
    ...
    if context.canEvaluatePolicy(policy, error: nil) {
      ...
      context.evaluatePolicy(policy, localizedReason: reason) { success, error in
        DispatchQueue.main.async {
          ...
          if success {
            ...
            ReactNativeBiometricDebug.debugLog(
              "authenticateWithOptions authentication failed: \(biometricsError.errorInfo.message)")
            handleError(biometricsError, reject: reject)
          }
        }
      }
      ...
      ReactNativeBiometricDebug.debugLog(
        "Biometric authentication not available - policy cannot be evaluated")
      handleError(.biometryNotAvailable, reject: reject)
    }
  }
  ...
  func createKeys(
    ...
    ReactNativeBiometricDebug.debugLog(
      "createKeys called with keyAlias: \(keyAlias ?? "default"), keyType: \(keyType ?? "ec256"), biometricStrength: \(biometricStrength ?? "strong")"
    )
    ...
    let deleteQuery = createKeychainQuery(
      keyTag: keyTag, includeSecureEnclave: biometricKeyType == .ec256)
    ...
    let keyAttributes = createKeyGenerationAttributes(
      keyTagData: keyTagData, accessControl: accessControl, keyType: biometricKeyType)
    ...
    guard let privateKey = SecKeyCreateRandomKey(keyAttributes as CFDictionary, &error) else {
      ...
      if let cfError = error?.takeRetainedValue() {
        ReactNativeBiometricDebug.debugLog(
          "createKeys failed - Key generation error: \(cfError.localizedDescription)")
      } else {
    ...
    ReactNativeBiometricDebug.debugLog(
      "Keys created successfully with tag: \(keyTag), type: \(biometricKeyType)")
    resolve(result)
  }
  ...
  func deleteKeys(
    ...
    case errSecSuccess:
      ...
      if verifyStatus == errSecItemNotFound {
        ...
        ReactNativeBiometricDebug.debugLog(
          "deleteKeys failed - Key still exists after deletion attempt")
    ...
    default:
      ReactNativeBiometricDebug.debugLog(
        "deleteKeys failed - Keychain error: status \(deleteStatus)")
  ...
  @objc
  func getAllKeys(
    _ customAlias: NSString?,
    resolver resolve: @escaping RCTPromiseResolveBlock,
    rejecter reject: @escaping RCTPromiseRejectBlock
  ) {
    ReactNativeBiometricDebug.debugLog(
      "getAllKeys called with customAlias: \(customAlias ?? "nil")")
    ...
    case errSecSuccess:
          ...
          let keyTagString = String(data: keyTag, encoding: .utf8)
        {
          ...
          if shouldIncludeKey {
            ...
            guard let keyRef = item[kSecValueRef as String] as! SecKey? else {
              ReactNativeBiometricDebug.debugLog(
                "Failed to get key reference for tag: \(keyTagString)")
              continue
            }
            ...
            if let publicKey = SecKeyCopyPublicKey(keyRef) {
              ...
              if let publicKeyString = exportPublicKeyToBase64(publicKey) {
                ...
                ReactNativeBiometricDebug.debugLog(
                  "Failed to export public key for tag: \(keyTagString)")
              }
            } else {
              ReactNativeBiometricDebug.debugLog(
                "Failed to get public key for tag: \(keyTagString)")
            }
          }
        }
      }
      ...
      ReactNativeBiometricDebug.debugLog(
        "getAllKeys completed successfully, found \(keysList.count) keys")
      resolve(resultDict)
  ...
  @objc
  func validateKeyIntegrity(
    _ keyAlias: NSString?,
    resolver resolve: @escaping RCTPromiseResolveBlock,
    rejecter reject: @escaping RCTPromiseRejectBlock
  ) {
    ReactNativeBiometricDebug.debugLog(
      "validateKeyIntegrity called with keyAlias: \(keyAlias ?? "default")")
    ...
    guard status == errSecSuccess else {
      if status == errSecItemNotFound {
        ...
        let biometricsError = ReactNativeBiometricsError.fromOSStatus(status)
        ReactNativeBiometricDebug.debugLog(
          "validateKeyIntegrity failed - Keychain error: \(biometricsError.errorInfo.message)")
    ...
    guard let keyItem = result as? [String: Any],
      let keyRefValue = keyItem[kSecValueRef as String]
    else {
    ...
    if let signature = SecKeyCreateSignature(keyRef, algorithm, testData as CFData, &error) {
      ...
      if let publicKey = SecKeyCopyPublicKey(keyRef) {
        let isValid = SecKeyVerifySignature(
          publicKey, algorithm, testData as CFData, signature, &error)
        ...
        ReactNativeBiometricDebug.debugLog(
          "validateKeyIntegrity - Public key extraction failed for verification.")
        checks["signatureTestPassed"] = false
        integrityResult["error"] =
          ReactNativeBiometricsError.publicKeyExtractionFailed.errorInfo.message
      }
    } else {
      let biometricsError: ReactNativeBiometricsError
      if let cfError = error?.takeRetainedValue() {
        ...
        ReactNativeBiometricDebug.debugLog(
          "validateKeyIntegrity - Signature test failed: \(cfError.localizedDescription)")
      } else {
        biometricsError = .signatureCreationFailed
        ReactNativeBiometricDebug.debugLog(
          "validateKeyIntegrity - Signature test failed: Unknown error")
      }
      checks["signatureTestPassed"] = false
      integrityResult["error"] = biometricsError.errorInfo.message
    }
  ...
  func verifyKeySignature(
    ...
    verifyKeySignatureWithEncoding(
      keyAlias, data: data, promptTitle: promptTitle, promptSubtitle: promptSubtitle,
      cancelButtonText: cancelButtonText, inputEncoding: "utf8", resolver: resolve, rejecter: reject
    )
  }

  @objc
  func verifyKeySignatureWithOptions(
    _ keyAlias: NSString?,
    data: NSString,
    promptTitle: NSString?,
    promptSubtitle: NSString?,
    ...
    verifyKeySignatureWithEncoding(
      keyAlias, data: data, promptTitle: promptTitle, promptSubtitle: promptSubtitle,
      cancelButtonText: cancelButtonText, inputEncoding: inputEncoding, resolver: resolve,
      rejecter: reject)
  }

  @objc
  func verifyKeySignatureWithEncoding(
    _ keyAlias: NSString?,
    data: NSString,
    promptTitle: NSString?,
    promptSubtitle: NSString?,
    ...
    ReactNativeBiometricDebug.debugLog(
      "verifyKeySignature called with keyAlias: \(keyAlias ?? "default"), inputEncoding: \(encoding)"
    )
    ...
    guard status == errSecSuccess else {
      let biometricsError = ReactNativeBiometricsError.fromOSStatus(status)
      ReactNativeBiometricDebug.debugLog(
        "verifyKeySignature failed - \(biometricsError.errorInfo.message)")
      resolve([
        "success": false, "error": biometricsError.errorInfo.message,
    ...
    if encoding.lowercased() == "base64" {
      guard let decodedData = Data(base64Encoded: data as String) else {
        ...
        resolve([
          "success": false, "error": "Invalid base64 data", "errorCode": "INVALID_INPUT_ENCODING",
        ])
        return
      }
    ...
    var error: Unmanaged<CFError>?
    guard let signature = SecKeyCreateSignature(keyRef, algorithm, dataToSign as CFData, &error)
    else {
      let biometricsError: ReactNativeBiometricsError
      if let cfError = error?.takeRetainedValue() {
        ...
        ReactNativeBiometricDebug.debugLog(
          "verifyKeySignature failed - \(cfError.localizedDescription)")
      } else {
        biometricsError = ReactNativeBiometricsError.signatureCreationFailed
        ReactNativeBiometricDebug.debugLog(
          "verifyKeySignature failed - Signature creation failed (unknown error)")
      }
      resolve([
        "success": false, "error": biometricsError.errorInfo.message,
        "errorCode": biometricsError.errorInfo.code,
  ...
  func validateSignature(
    ...
    ReactNativeBiometricDebug.debugLog(
      "validateSignature called with keyAlias: \(keyAlias ?? "default")")
    ...
    guard status == errSecSuccess else {
      let biometricsError = ReactNativeBiometricsError.fromOSStatus(status)
      ReactNativeBiometricDebug.debugLog(
        "validateSignature failed - \(biometricsError.errorInfo.message)")
    ...
    guard let signatureData = Data(base64Encoded: signatureString) else {
      ReactNativeBiometricDebug.debugLog(
        "validateSignature failed - Invalid base64 signature format. Length: \(signatureString.count), First 20 chars: \(String(signatureString.prefix(20)))"
      )
    ...
    let isValid = SecKeyVerifySignature(
      publicKey, algorithm, dataToVerify as CFData, signatureData as CFData, &error)

    if let cfError = error?.takeRetainedValue() {
      let biometricsError = ReactNativeBiometricsError.signatureVerificationFailed
      ReactNativeBiometricDebug.debugLog(
        "validateSignature failed - \(cfError.localizedDescription)")
      resolve(["valid": false, "error": biometricsError.errorInfo.message])
    } else {
  ...
  @objc
  func getKeyAttributes(
    _ keyAlias: NSString?,
    resolver resolve: @escaping RCTPromiseResolveBlock,
    rejecter reject: @escaping RCTPromiseRejectBlock
  ) {
    ReactNativeBiometricDebug.debugLog(
      "getKeyAttributes called with keyAlias: \(keyAlias ?? "default")")
    ...
    guard status == errSecSuccess else {
      if status == errSecItemNotFound {
        ...
        ReactNativeBiometricDebug.debugLog(
          "getKeyAttributes - Key exists but authentication required (UI disabled)")
        resolve([
          "exists": true,
          "attributes": [
        ...
        let biometricsError = ReactNativeBiometricsError.fromOSStatus(status)
        ReactNativeBiometricDebug.debugLog(
          "getKeyAttributes failed - \(biometricsError.errorInfo.message)")
        resolve([
          "exists": false, "error": biometricsError.errorInfo.message,
          "errorCode": biometricsError.errorInfo.code,
        ])
      }
      return
  ...
  func getDeviceIntegrityStatus(
    ...
    ReactNativeBiometricDebug.debugLog(
      "Device integrity check completed - isCompromised: \(integrityStatus["isCompromised"] as? Bool ?? false)"
    )
