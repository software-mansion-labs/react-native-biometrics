 ...
 *     Uses `.userPresence` (biometry OR passcode) instead of `.biometryAny` (biometry only).
 *     The Secure Enclave is still used for key storage regardless of this setting.
...
.biometryAny,
    ...
    // For EC keys (in Secure Enclave), we use .privateKeyUsage combined with the auth constraint
    return SecAccessControlCreateWithFlags(
      kCFAllocatorDefault,
